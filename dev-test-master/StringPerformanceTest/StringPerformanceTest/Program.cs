...
namespace StringPerformanceTest
{
    ...
    public class Benchmark
    {
        ...
        public int CountOccurrences()
        {
            ...
            for (int i=0; i < SearchValue[i].Length;i++)
            {
                for (int f=0; f < FileToRead[f].Length;f++)
                {
                    int occurance = FileToRead[f].Count(s => s == SearchValue[i]);
                    Console.WriteLine(SearchValue[i] has + occurance + " occurances in " + FileToRead[f]);
                }
            }
